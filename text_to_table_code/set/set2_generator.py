...
class Set2Generator(SequenceGenerator):
    def __init__(
        self,
        models,
        tgt_dict,
    ...
    def _generate(
        ...
        if self.ground_label:
            classes_pred = sample["label"]
        else:
            classes_dists = self.model.forward_classification_head(src_tokens, encoder_outs)
            classes_pred = classes_dists.sigmoid() > 0.5
        num_classes = classes_pred.size(1)
        beam_segment_ids = torch.full((bsz, num_classes+2), num_classes+1, dtype=torch.long, device=src_tokens.device)
        beam_segment_ids[:, 0] = 0
        beam_segment_i = torch.zeros(bsz, dtype=torch.long, device=src_tokens.device)
        for b in range(bsz):
            c = 1
            for i in range(classes_pred.size(1)):
                if classes_pred[b, i] == 1:
                    beam_segment_ids[b, c] = i+1
                    c += 1
        ...
        if isinstance(self.model.models[0], BARTModel):
            beam_segment_ids = beam_segment_ids.unsqueeze(1).repeat(1, max_num*beam_size, 1).reshape(bsz*max_num*beam_size, num_classes+2)
            beam_segment_i = torch.zeros(bsz*max_num*beam_size, dtype=torch.long, device=src_tokens.device)
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                ...
                beam_segment_ids = beam_segment_ids[reorder_state]
                beam_segment_i = beam_segment_i[reorder_state]
                beam_segment_i[tokens[:, step] == self.sep] += 1
                self.model.reorder_incremental_state(incremental_states, reorder_state)
                encoder_outs = self.model.reorder_encoder_out(
                    encoder_outs, reorder_state
                )
            segment_ids = beam_segment_ids.gather(-1, beam_segment_i.unsqueeze(-1))
        ...
        for i in range(origin_bsz):
            ...
            if self.unique_row_header:
                finalized_i[0]['row_score'] = torch.tensor([finalized[i*max_num+j][0]['score'] for j in range(max_num)])
            else:
        ...
        return new_finalized, classes_pred.long()
...
class Set2EnsembleModel(EnsembleModel):
    
    def forward_classification_head(self, src_tokens, encoder_outs):
        classes_dists = [model.forward_classification_head(src_tokens, encoder_out) for model, encoder_out in zip(self.models, encoder_outs)]
        if len(classes_dists) > 1:
            classes_dists = torch.stack(classes_dists, dim=0).mean(dim=0)
        else:
